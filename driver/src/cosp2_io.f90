...
contains
  ...
  subroutine write_cosp2_output(Npoints, Ncolumns, Nlevels, lev, lon, lat, cospOUT, outFileName)
    ...
    if (associated(cospOUT%cfodd_ntotal)) then
       ...
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_def_var(fileID,"ncfodd4",nf90_float, (/dimID(1),dimID(17),dimID(18)/),varID(146))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(146),"long_name","# of CFODD (all Reff range)")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(146),"units",        "1")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(146),"standard_name", "cfodd_reff_all")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       !! axes for CFODD
       status = nf90_def_var(fileID,"CFODD_NDBZE",nf90_float,(/dimID(17)/),varID(147))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(147),"long_name","CloudSat+MODIS dBZe vs ICOD joint PDF X-axis")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(147),"units",        "dBZ")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(147),"standard_name", "cloudsat_quivalent_reflectivity_factor")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_def_var(fileID,"CFODD_NICOD",nf90_float,(/dimID(18)/),varID(148))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(148),"long_name","CloudSat+MODIS dBZe vs ICOD joint PDF Y-axis")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(148),"units",        "none")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(148),"standard_name", "modis_in-cloud_optical_depth")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
    endif 
    ...
    if (associated(cospOUT%cfodd_ntotal)) then
       ...
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_var(fileID,varID(146),cospOUT%cfodd_ntotal(:,:,:,4))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_var(fileID,varID(147),CFODD_HISTDBZEcenters)
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_var(fileID,varID(148),CFODD_HISTICODcenters)
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
    endif
