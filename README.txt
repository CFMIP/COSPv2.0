...
and produces netCDF output. The layer mimimic the COSP 1.4 interface is tested
with a separate driver. A third driver uses the CMOR infrastructure to write a
...
4) Running the offline tests
  ...
  a) Build the drivers.
    ...
    *) In build 'make driver' will build a COSP library, a separate library with
    ...
    analogous but build a cosp1_test executable that uses the COSP 1.4 calling
    conventions.

  b) Running the test program
    *) Directory test/run contains namelists and other files needed by the test
    programs. If the executables have been built they should run with in this
    ...
    *) The behavior of COSP can be changed via the input (driver/src/cosp2_input_nl.txt)
    ...
    controls the COSP setup (i.e. Number of subcolumns to beused, etc...) and
  ...
  c) Regression testing (comparing to reference data)
    *) [DUSTIN TK] Invoke Python script in driver/. Python dependencies. 
    Options for --cmor.
